$(document).ready(function(){
		...
		$("#email").change(function(){
			if($("#email").val()==""){		// 직접 입력 선택
			  //$("#domain").attr("readonly", false); // 읽기전용 해제 방법1
				$("#domain").removeAttr("readonly");  // 일기전용 해제 방범2
				$("#domain").val("").focus();
			}else{							// 도메인 선택
		...
		$("form").submit(function(){
			...
			/* 방법1
			var cnt = 0;
			if($("#h1").is(":checked")) cnt+=1;
			if($("#h2").is(":checked")) cnt+=1;
			if($("#h3").is(":checked")) cnt+=1;
			if($("#h4").is(":checked")) cnt+=1;
			if($("#h5").is(":checked")) cnt+=1;
